<template>
    <div class="p-4 sm:ml-64">
            ...
            <div class="w-full max-w-6xl mx-auto bg-white shadow-lg rounded-sm border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                <header class="px-5 py-4 border-b border-gray-100 flex align-middle justify-between">
                    ...
                    <router-link v-if="checkPermission('role-create')" :to="'users/add'" class="inline-flex items-center justify-center w-full px-4 py-3 mb-2 text-sm text-white bg-green-500 rounded-md hover:bg-green-400 sm:w-auto sm:mb-0" data-primary="green-400" data-rounded="rounded-2xl" data-primary-reset="{}">
                        เพิ่มข้อมูล
                    </router-link>
                </header>
                <div class="p-3">
                        ...
                        <tbody class="text-sm ">
                            <tr v-for="user in permission_list.data" :key="user">
                                ...
                                <div class="flex items-center">
                                    <div class="font-medium text-gray-800 dark:text-white">{{ user.title }}</div>
                                </div>
                                </td>
                                <td class="p-2 whitespace-nowrap">
    ...
    <script setup>
        import RolesController from '@/plugins/RolesController'
        import { onMounted, ref } from 'vue'
    
        const { getDataWithTable, fetchData } = RolesController()
