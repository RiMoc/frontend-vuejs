   ...
   <div class="flex flex-col justify-center h-fit">
      <div class="w-full max-w-6xl mx-auto bg-white shadow-lg rounded-sm border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
         <header class="px-5 py-4 border-b border-gray-100 flex align-middle justify-between">
            <h2 class="font-semibold text-gray-800 dark:text-gray-50">{{ props.title }}</h2>
            <router-link :to="props.route+ '/add'" class="inline-flex items-center justify-center w-full px-4 py-3 mb-2 text-sm text-white bg-green-500 rounded-md hover:bg-green-400 sm:w-auto sm:mb-0" data-primary="green-400" data-rounded="rounded-2xl" data-primary-reset="{}">
                เพิ่มข้อมูล
            </router-link>
         </header>
         <div class="p-3">
            <div class="overflow-x-auto">
               <table class="table-auto w-full ">
                  ...
                  <tbody class="text-sm ">
                     <tr v-for="user in props.data.data" :key="user">
                ...
                <nav class="flex justify-end py-5">
                    <ul class="list-style-none flex">
                        <li v-for="pi in props.data.links" :key="pi">
                            <a  :class='{ "relative block rounded bg-primary-100 px-3 py-1.5 text-sm font-medium text-primary-700 transition-all duration-300": pi.active, "relative block rounded bg-transparent px-3 py-1.5 text-sm text-neutral-600 transition-all duration-300 hover:bg-neutral-100  dark:text-white dark:hover:bg-neutral-700 dark:hover:text-white": !pi.active }'>
...
<script setup>
const props = defineProps(['data', 'title', 'route'])
</script>